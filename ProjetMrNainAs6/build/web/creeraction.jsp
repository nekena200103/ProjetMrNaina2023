...
<body>
  <div class="container-scroller">
    ...
    ArrayList<Action> array=(ArrayList<Action>)request.getAttribute("listeObj");
    ...
    <nav class="navbar col-lg-12 col-12 p-0 fixed-top d-flex flex-row">
      ...
      <div class="navbar-menu-wrapper d-flex align-items-center justify-content-end">
        ...
        <div class="col-lg-6 grid-margin stretch-card">
          <div class="card">
            <div class="card-body">
              <h4 class="card-title">Voici la liste de tous les actions que comporte cette scene:<%= scene.getDescription()%></h4>
                ...
                <table class="table">
                  <thead>
                    <tr>
                      ...
                      <th>Selectionnner</th>
                      <th>Duree de tournage estimee en heure</th>
                    </tr>
                  </thead>
                      ...
                      Action fil=(Action)array.get(i);%>
                       
                      <tr>
                      <td><%out.println(fil.getDescription_action()); %></td>
                      <td><%out.println(i+3*(numPage-1)); %></td>
                      <td><a href="<%= request.getContextPath()%>/listreplique?action=<% out.println(fil.getIdaction());%>""><button class="btn btn-success">Selectionner</button></a></td>
                      <td><%out.println(fil.getDuree()); %></td>
                        </tr>
                      <%
        ...
        <ul class="pagination">
            <%if (numPage-1!=0) {%>
                <li class="page-item"><a class="page-link" aria-label="Previous" href="<%= request.getContextPath() %>/listaction?sessionrecherche=<%= sessionrecherche %>&numPage=<%= numPage-1 %>&scene=<%= scene.getIdscene()%>"><span aria-hidden="true">«</span></a></li>
                 <%}%>
            <%for (int i=1;i<nombrePage+1;i++) {%>
            
            <li class="page-item"><a class="page-link" href="<%= request.getContextPath() %>/listaction?sessionrecherche=<%= sessionrecherche %>&numPage=<%= i %>&scene=<%= scene.getIdscene()%>"><%= i %></a></li>
            <%}%>
            
            <%if (ariana==1) {%>
            <li class="page-item"><a class="page-link" aria-label="Next" href="<%= request.getContextPath() %>/listaction?sessionrecherche=<%= sessionrecherche %>&numPage=<%= numPage+1 %>&scene=<%= scene.getIdscene()%>"><span >»</span></a></li>
            <%}%>
        </ul>
    </nav>
              </div>
        ...
        <div class="col-lg-6 grid-margin stretch-card">
            <div class="card">
                <div class="card-body">
                  <h4 class="card-title">Creer une Action ici </h4>
                  <p class="card-description">
                    
                  </p>
                  <form class="forms-sample" action="<%= request.getContextPath() %>/insertionaction" method="POST" enctype="multipart/form-data">
                    <div class="form-group">
                      <label for="exampleInputUsername1">Description de l'action</label>
                      <input type="text" class="form-control" id="exampleInputUsername1" name="description_action">
                      <br>
                      <label for="exampleInputUsername1">Duree estimee en heure</label>
                      <input type="number" class="form-control" id="exampleInputUsername1" name="duree">
