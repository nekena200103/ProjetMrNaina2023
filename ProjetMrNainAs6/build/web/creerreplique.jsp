...
<body>
  <div class="container-scroller">
    ...
    ArrayList<Replique> array=(ArrayList<Replique>)request.getAttribute("listeObj");
    ArrayList<Personne> pers=(ArrayList<Personne>)request.getAttribute("personne");
    Action scene=(Action)request.getAttribute("action");
    ...
    <nav class="navbar col-lg-12 col-12 p-0 fixed-top d-flex flex-row">
      ...
      <div class="navbar-menu-wrapper d-flex align-items-center justify-content-end">
        ...
        <div class="col-lg-6 grid-margin stretch-card">
          <div class="card">
            <div class="card-body">
              <h4 class="card-title">Voici la liste de toutes les repliques que comporte cette action:<%= scene.getDescription_action()%></h4>
              ...
              <div class="table-responsive">
                <table class="table">
                  <thead>
                    <tr>
                      ...
                      Replique fil=(Replique)array.get(i);%>
                       
                      <tr>
                      <td><%out.println(fil.getReplique()); %></td>
                      <td><%out.println(i+3*(numPage-1)); %></td>
                      <td><a href=""><button class="btn btn-success">Selectionner</button></a></td>
                ...
                <nav style="margin-left: 10%; width: 20vw;">
        <ul class="pagination">
            <%if (numPage-1!=0) {%>
                <li class="page-item"><a class="page-link" aria-label="Previous" href="<%= request.getContextPath() %>/listreplique?sessionrecherche=<%= sessionrecherche %>&numPage=<%= numPage-1 %>&action=<%= scene.getIdaction()%>"><span aria-hidden="true">«</span></a></li>
                 <%}%>
            <%for (int i=1;i<nombrePage+1;i++) {%>
            
            <li class="page-item"><a class="page-link" href="<%= request.getContextPath() %>/listreplique?sessionrecherche=<%= sessionrecherche %>&numPage=<%= i %>&action=<%= scene.getIdaction()%>"><%= i %></a></li>
            <%}%>
            
            <%if (ariana==1) {%>
            <li class="page-item"><a class="page-link" aria-label="Next" href="<%= request.getContextPath() %>/listreplique?sessionrecherche=<%= sessionrecherche %>&numPage=<%= numPage+1 %>&action=<%= scene.getIdaction()%>"><span >»</span></a></li>
            <%}%>
        </ul>
    </nav>
              </div>
        ...
        <div class="col-lg-6 grid-margin stretch-card">
            <div class="card">
                <div class="card-body">
                  <h4 class="card-title">Creer une Replique  ici </h4>
                  <p class="card-description">
                    
                  </p>
                  <form class="forms-sample" action="<%= request.getContextPath() %>/insertionreplique" method="POST" enctype="multipart/form-data">
                    <div class="form-group">
                      <label for="exampleInputUsername1">Entrer la replique</label>
                      <input type="text" class="form-control" id="exampleInputUsername1" name="replique">
                      <input type="hidden" class="form-control" id="exampleInputUsername1" name="idaction" value="<%= scene.getIdaction()%>">
                      <br>
                      <select class="form-select" style="width: 80%;margin-left: 10%;margin-top: 3vh;" name="idpersonne">
            <%for (Personne elem : pers) {%>
                <option value="<%=elem.getIdpersonne()%>"><%=elem.getNom()%></option>
            <%}%>
        </select>
                      <input type="hidden" class="form-control" id="exampleInputUsername1" name="idscene" value="<%= scene.getIdscene()%>">
                    </div>
